<img src="https://github.com/user-attachments/assets/35520617-8ce6-4180-a5a0-1327fd052cff" alt="Banner GitHub" width="100%"/>

[![Star](https://img.shields.io/github/stars/Jesus-David-Silva-Rangel-19/Housing-Price-Analysis?style=social)](https://github.com/Jesus-David-Silva-Rangel-19)
[![Fork](https://img.shields.io/github/forks/Jesus-David-Silva-Rangel-19/Housing-Price-Analysis?style=social)](https://github.com/Jesus-David-Silva-Rangel-19)
[![Issues](https://img.shields.io/github/issues/Jesus-David-Silva-Rangel-19/Housing-Price-Analysis)](https://github.com/Jesus-David-Silva-Rangel-19)
[![Buy Me a Coffee](https://img.shields.io/badge/Buy%20Me%20a%20Coffee-donate-yellow.svg)](https://buymeacoffee.com/jesus.david.rangel)


# 👋 ¡Hola! Soy **Jesús David Silva Rangel**

### Analista de Datos | Administrador de Bases de Datos | Aspirante a Científico de Datos  

Apasionado por los datos, la estadística y la automatización de procesos. Actualmente estudio **Ingeniería de Sistemas**, mientras desarrollo proyectos de análisis y visualización de datos para optimizar la toma de decisiones empresariales.  


## 🧠 Sobre mí

- 🗄️ Administrador de bases de datos  
- 📊 Analista de datos con enfoque en **automatización y análisis estadístico**  
- 🗂️ Experiencia en reportes, dashboards y visualización  
- 🎓 Estudiante de **Ingeniería de Sistemas**  
- 🚀 En camino a convertirme en **Data Scientist** y **Machine Learning Engineer**  

[![GitHub Streak](https://streak-stats.demolab.com?user=Jesus-David-Silva-Rangel-19&theme=default)](https://git.io/streak-stats)


## 💼 Lo que hago actualmente

- Creación de reportes y paneles interactivos en **Power BI** y **Excel**  
- Análisis estadístico de datos demográficos y poblacionales  
- Administración y optimización de **bases de datos relacionales**  
- Desarrollo de scripts para **automatización de procesos** administrativos  
- Análisis de datos orientado a la **toma de decisiones estratégicas**

## 💻 Habilidades técnicas

**Lenguajes de Programación:**  
Python · SQL · PL/SQL  

**Librerías y Frameworks:**  
Pandas · NumPy · Matplotlib · Seaborn · SciPy · StatsModels · Scikit-Learn  

**Bases de Datos:**  
MySQL · PostgreSQL · Oracle Apex · MongoDB · SQL Workbench  

**Visualización de Datos:**  
Power BI · Tableau · Matplotlib · Seaborn  

**Big Data:**  
Spark · PySpark · Hadoop  

**Herramientas de Desarrollo:**  
Jupyter Notebook · Visual Studio Code · Git · GitHub · CLI (Linux / Windows)  

**Productividad y Colaboración:**  
Excel · Google Sheets · Notion · Jira · Slack · Asana · Trello · Todoist  

**Metodologías:**  
CRISP-DM · ASUM-DM · SEMMA · Scrum · Kanban  


## 🧩 Responsabilidades y experiencia práctica

- Limpieza, transformación y análisis de datos con **Python (Pandas, NumPy)**  
- Diseño y optimización de consultas SQL en entornos **relacionales**  
- Creación de dashboards y reportes dinámicos en **Power BI**  
- Aplicación de modelos **estadísticos y predictivos**  
- Desarrollo de **pipelines ETL** para automatizar procesos de integración de datos  
- Visualización efectiva de datos con **Matplotlib y Seaborn**  
- Documentación técnica y narrativa en **Jupyter Notebooks**  
- Colaboración ágil en equipos multidisciplinarios  
- Control de versiones con **Git y GitHub**

## ☕ Apoya mi trabajo

Cada análisis lleva consigo una buena dosis de café.  
Si te gusta mi trabajo, puedes invitarme uno aquí:  

👉 [**Buy Me a Coffee**](https://buymeacoffee.com/jesus.david.rangel)

## 📫 Contacto

- ✉️ **Correo:** [jdsr.cuentaprofesional@gmail.com](mailto:jdsr.cuentaprofesional@gmail.com)  
- 💼 **LinkedIn:** [Jesús David Silva Rangel](https://www.linkedin.com/in/jes%C3%BAs-david-silva-rangel-77706a260/)  
- 🐦 **X / Twitter:** [@DavidSilvaR19](https://x.com/DavidSilvaR19)  
- 📞 **WhatsApp:** [(+57) 322 960 8830](https://wa.link/tje6u7)  

✨ *"Transformando datos en conocimiento, y el conocimiento en decisiones."*
